<?php
    ...
    if ($count > 0) 
    {
        ...
        if ($updateResult) {
           ...
           redirect('../../Staff UI/make_orders.php?id='.$res_id.'&status=1', '', "You've ordered {$qty}  {$product['data']['prod_name']} successfully!");
        } 
        else 
        {
            //echo "Lỗi khi thực hiện UPDATE: " . $updateStmt->error;
            redirect('../../Staff UI/make_orders.php?id='.$res_id.'&status=1', 'Something went wrong! Please try again...', "");
        }
