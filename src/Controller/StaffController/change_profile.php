...
if (isset($_POST['changeAdminProfile'])) {
    ...
    if (empty($_POST["admin_phone"]) || empty($_POST["admin_fullname"]) || empty($_POST['admin_email'])) {
        ...
        if($conn->query($sql) === TRUE){
            redirect('../../Staff UI/change_profile.php', '', "You've successfully changed the profile");
        }
        else{
            redirect('../../Staff UI/change_profile.php', "You haven't successfully changed the profile", "");
        }
    }
}
