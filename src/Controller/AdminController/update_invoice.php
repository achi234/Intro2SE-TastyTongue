<?php
    ...
    if(isset($_POST['btn-update-invoice']))
    {
        ...
        if(empty($reservation_id) || empty($payment_id) || !isset($status))
        {
            ...
            redirect('../../Admin UI/update_invoices.php?id='.$invoice_id, 'All fields are required.', '');
            exit(0);
        }
        else
        {
            ...
            if ($count > 0) {
                ...
                if ($updateResult) {
                    ...
                    redirect('../../Admin UI/invoices.php', '', "You've update invoice {$invoice_id}!");
                } else {
                    echo "Lỗi khi thực hiện UPDATE: " . $updateStmt->error;
                    redirect('../../Admin UI/update_invoices.php?id='.$invoice_id, '', "Something went wrong! Please enter again...");
                }
